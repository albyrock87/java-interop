	...
	public sealed partial class JniInstanceMethods
	{
		...
		public JniMethodInfo GetConstructor (string signature)
		{
			...
			lock (InstanceMethods) {
				ref var m = ref CollectionsMarshal.GetValueRefOrAddDefault(InstanceMethods, signature, out var exists);
				if (!exists) {
		...
		internal JniInstanceMethods GetConstructorsForType (Type declaringType)
		{
			...
			lock (SubclassConstructors) {
				ref var m = ref CollectionsMarshal.GetValueRefOrAddDefault(SubclassConstructors, declaringType, out var exists);
				if (!exists) {
					m = new JniInstanceMethods (declaringType);
				}
		...
		{
			lock (InstanceMethods) {
				ref var m = ref CollectionsMarshal.GetValueRefOrAddDefault(InstanceMethods, encodedMember, out var exists);
				if (!exists) {
					JniPeerMembers.GetNameAndSignature (encodedMember, out var method, out var signature);
					m = GetMethodInfo (method, signature);
				}
